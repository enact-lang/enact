...
namespace enact {
    ...
    std::vector<std::unique_ptr<Stmt>> Analyser::analyse(std::vector<std::unique_ptr<Stmt>> ast) {
        ...
        if (m_scopes.empty()) {
            ...
            declareVariable("print",
                            Variable{
                                    std::make_shared<FunctionType>(NOTHING_TYPE, std::vector<Type>{DYNAMIC_TYPE}, false,
                                                                   true), true});
            declareVariable("put",
                            Variable{
                                    std::make_shared<FunctionType>(NOTHING_TYPE, std::vector<Type>{DYNAMIC_TYPE}, false,
                                                                   true), true});
            declareVariable("dis",
                            Variable{std::make_shared<FunctionType>(STRING_TYPE, std::vector<Type>{DYNAMIC_TYPE}, false,
                                                                    true), true});
        }
    ...
    void Analyser::visitGetExpr(GetExpr &expr) {
        ...
        if (objectType->isStruct()) {
            ...
            throw errorAt(expr.name, "Struct type '" + type->getStructType()->getName() +
    ...
    void Analyser::visitSetExpr(SetExpr &expr) {
        ...
        if (!expr.target->getType()->looselyEquals(*expr.value->getType())) {
            throw errorAt(expr.oper, "Cannot assign value of type '" + expr.value->getType()->toString()
                                     + "' to property '" + expr.target->name.lexeme + "' of type '" +
                                     expr.value->getType()->toString()
                                     + "'.");
        }
    ...
    void Analyser::visitTernaryExpr(TernaryExpr &expr) {
        ...
        if (!expr.thenExpr->getType()->looselyEquals(*expr.elseExpr->getType())) {
            throw errorAt(expr.oper,
                          "Mismatched types in conditional operation: '" + expr.thenExpr->getType()->toString()
